use super::{
    gamelog::GameLog, AtkBonus, CombatStats, DefBonus, Equipped, Name, SufferDamage, WantsToMelee,
};
use specs::prelude::*;
            ...
            (&entities, &wants_melee, &names, &combat_stats).join()
        {
            if stats.hp > 0 {
                ...
                for (_item_entity, power_bonus, equipped_by) in
                    (&entities, &atk_bonuses, &equipped).join()
                {
                ...
                if target_stats.hp > 0 {
                    ...
                    for (_item_entity, def_bonus, equipped_by) in
                        (&entities, &def_bonuses, &equipped).join()
                    {
